    ...
    constructor(context: vscode.ExtensionContext, snippet: Snippet, snippetsProvider: SnippetsProvider) {
        this._panel = vscode.window.createWebviewPanel(
            ...
            },
            {
                ...
                localResourceRoots: [vscode.Uri.file(path.join(context.extensionPath, EditSnippetFolder.viewsFolder))]
            }
        );
        ...
        const resourceName = "editSnippetFolder";
        const htmlTemplate = path.join(context.extensionPath, EditSnippetFolder.viewsFolder, `${resourceName}.html`);
        this._panel.webview.html = mustache.render(fs.readFileSync(htmlTemplate).toString(),
            {
                cspSource: this._panel.webview.cspSource,
                resetCssUri: this._panel.webview.asWebviewUri(vscode.Uri.file(path.join(context.extensionPath, EditSnippetFolder.viewsFolder, 'css', 'reset.css'))),
                cssUri: this._panel.webview.asWebviewUri(vscode.Uri.file(path.join(context.extensionPath, EditSnippetFolder.viewsFolder, 'css', 'vscode-custom.css'))),
                jsUri: this._panel.webview.asWebviewUri(vscode.Uri.file(path.join(context.extensionPath, EditSnippetFolder.viewsFolder, 'js', `${resourceName}.js`))),
                snippet: snippet,
                docsUrl: EditSnippetFolder.docsUrl
            }
        );
